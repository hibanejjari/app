...
def in_progress_by_user_logic(df):
    df = df.copy()
    df["Latest Status"] = df["Latest Status"].astype(str).str.upper()
    df["Approver Action"] = df["Approver Action"].astype(str).str.upper()

    in_progress = df[df["Latest Status"].str.contains("PROGRESS", na=False)]
    in_progress = in_progress[~in_progress["Approver Action"].isin(["APPROVED", "CANCELLED"])]
    ...
    next_approver = (
        in_progress
        .sort_values(by=["Purchase Order No.", "Approver Level"])
        .groupby("Purchase Order No.", as_index=False)
        .first()
    )

    return next_approver["Approver Name"].value_counts().sort_index()
        ...
        if selected_graphs:
            for title in selected_graphs:
                if title == "📋 Dashboard Summary":
                    ...
                    chart_images.append((title, "DASHBOARD_TABLE"))
                    continue
                ...
                if isinstance(chart_data, pd.Series):
                    chart_data = chart_data.to_frame(name="Value")
                ...
                sorted_data = chart_data.sort_values(by=chart_data.columns[0])
                bars = sorted_data.plot(kind="bar", ax=ax, legend=False)
